...
However, it was still possible to accurately measure organizational contributions, for the following reason: through experimentation it was found that the vast majority of commits to a project typically came from a small subset of the committers; since these contributors were the most active, it was sufficient to identify only their employers and ignore the others, without damaging the validity of the results. For this reason, the data collection was limited to only the prolific contributors of each project, defined as contributors in the minimum size set S such that the sum of the number of commits done by members of S is at least 80\% of the number of commits done to the project during \timeperiod. This also had the effect of improving the proportion of employer names mined automatically, because prolific contributors seem to be more likely to keep their Git, Github, and JIRA profile information up-to-date.
...
\begin{enumerate}
	...
	\item Associate accounts belonging to a single person under a single contributor ID (See section \ref{mergingsec}) \pd{Put forward references to 2.5.4, using ref command} \bmdone{I explain it in 2.5.4} \bm{Done.}
	\item Generate a guess for the employer of each contributor (See section \ref{employersec}) \pd{Forward references here} \bmdone{I explain it in 2.5.5} \bm{Done.}
\end{enumerate}
...
\pddone{Use consistent typography for "JIRA"} \bmdone{In this context, jira is the package name, so it has the proper case. Is there a special way to typeset Python package names to make it more clear?}
\pd{Jira is being used as a noun modifier, here so it's OK to use the same font here as everywhere} \bm{Fixed.}
