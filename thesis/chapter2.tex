...
\begin{figure}
	...
	\centering
	\caption{The relevant tables of the GHTorrent database \pd{Show foreign keys with asterix, and indicate this in this caption}}
...
\begin{enumerate}
	...
	\item Associate accounts belonging to a single person under a single contributor ID \pd{Put forward references to 2.5.4, using ref command} \bmdone{I explain it in 2.5.4}
	\item Generate a guess for the employer of each contributor \pd{Forward references here} \bmdone{I explain it in 2.5.5}
\end{enumerate}
...
\pd{Jira is being used as a noun modifier, here so it's OK to use the same font here as everywhere}
