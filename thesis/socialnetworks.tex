...
\begin{enumerate}
	...
	\item The edge list was imported into R, symmetric edges were removed, and edges connecting the same vertices were merged by summing their commit counts. \pd{Briefly explain why multiple edges may arise between a single pair of nodes} 
...
\end{equation*}
where where d(i,j) is the geodesic distance between i and j (where defined)\cite{butts}. \pd{Explain geodesic, and when it might not be defined} 
...
\pd{Need to provide a more formal definition of how it is calculated} 
...
The GN algorithm identified sixteen communities, or clusters, in the organizational network. Since there were forty projects, it is clear that the grouping algorithm is more sophisticated than simply grouping organizations with their main projects. \pd{The previous sentence needs more details and justification. Add a few sentences explaining why you find that GN is doing something sophisticated}However, project collaborations are what the network is based on, so as a first attempt at understanding these clusters, table \ref{tab:clusterprojects} relates clusters to the projects they are most associated with.\pd{Don't end sentences a proposition with :-)}
