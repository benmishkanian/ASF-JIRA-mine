...
\begin{enumerate}
	...
	\item If two edges connected the same pair of vertices (caused by two organizations being connected through multiple projects), the edges were merged by summing their commit counts. \pd{Briefly explain why multiple edges may arise between a single pair of nodes} \bm{Done.}
...
\end{equation*}
This is the ``Freeman'' degree---the sum of the indegree and outdegree of the vertex v. A table of the organizations with the highest degree centrality is shown in table \ref{tab:degree}. \pd{Provide a fomula for degree centrality}. \bm{Done.}
...
\end{equation*}
where d(i,j) is ``the geodesic distance between i and j (where defined)''\cite{butts}. A geodesic is the shortest path between two vertices. If there is no path between the vertices, the geodesic is undefined. \pd{Explain geodesic, and when it might not be defined} \bm{Done.}
...
\begin{table}
	\begin{tabular}{l|c}
		...
		\hline
		Salesforce, Gruter, Twitter, etc. & bigtop \\
		\hline
		Docker & aurora \\
		\hline
		DataStax & cassandra \\
		...
		\hline
		Hortonworks, IBM, Confluent, etc. & spark, storm, kafka, oozie, bigtop, flink, hbase, samza \\
		\hline
		...
		\hline
		JDriven, Energized Work, Ampool, etc. & groovy
	\end{tabular}
